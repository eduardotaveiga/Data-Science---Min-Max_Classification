...
from MinMaxClassification.obj_functions_classifiers import randomForest_objective_function
...
rf_time_money_test = []
rf_time_money_train = []
rf_money_earned_train = []
rf_money_earned_test = []
for stock in stocks:
    ...
    clf, feature_subset = ModelTraining(train_df, clf, features,
                                        objective_function=randomForest_objective_function,
                                        scoring='roc_auc_ovo', cross_val_iterations=1, optimize_trials=2,
                                        scale_data=False).return_model()

    # backtest
    money_train, trades_train, values_train = Simulation(train_df[feature_subset], train_df['target'], clf,
                                                         scale_data=False).run()
    money_test, trades_test, values_test = Simulation(test_df[feature_subset], test_df['target'], clf,
                                                      scale_data=False).run()

    rf_money_earned_train.append(money_train)
    rf_money_earned_test.append(money_test)
    rf_time_money_train.append(values_train)
    rf_time_money_test.append(values_test)

    print('------------------------------------------------------------------------------------------------------')

data = {'Stock':stocks, 'Train_set':rf_money_earned_train, 'Test_set':rf_money_earned_test}
